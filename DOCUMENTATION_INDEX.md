# Documentation Index

**v2.1.0** | Dec 5, 2025

---

## Root Docs

| Doc | Purpose |
|-----|---------|
| [README](README.md) | Quick start |
| [INSTALL](INSTALL.md) | Full install guide |
| [CLAUDE.md](CLAUDE.md) | Developer reference |
| [SPECIFICATIONS](SPECIFICATIONS.md) | 109 FRs |
| [ROADMAP](ROADMAP.md) | Project status |
| [CHANGELOG](CHANGELOG.md) | Version history |
| [FAQ](FAQ.md) | Common questions |
| [SECURITY](SECURITY.md) | Security policy |

---

## docs/

| Doc | Purpose |
|-----|---------|
| [ARCHITECTURE](docs/ARCHITECTURE.md) | System design |
| [RELEASE_NOTES_v2.1.0](docs/RELEASE_NOTES_v2.1.0.md) | Release notes |

### docs/user/

| Doc | Purpose |
|-----|---------|
| [user-guide](docs/user/user-guide.md) | Full user guide |
| [troubleshooting](docs/user/troubleshooting.md) | Common issues |
| [github-integration](docs/user/github-integration.md) | GitHub CLI |
| [ollama-chat](docs/user/ollama-chat.md) | AI chat |
| [performance-tips](docs/user/performance-tips.md) | Optimization |

### docs/developer/

| Doc | Purpose |
|-----|---------|
| [contributing](docs/developer/contributing.md) | How to contribute |
| [testing](docs/developer/testing.md) | Test guide |
| [ma-principle](docs/developer/ma-principle.md) | Code organization |
| [security-guide](docs/developer/security-guide.md) | Security practices |
| [configuration](docs/developer/configuration.md) | Config guide |

---

## Metrics

| Metric | Value |
|--------|-------|
| Source | 45,900 lines / 180 files |
| Unit tests | 5,122 |
| E2E tests | 17 |
| FRs | 109 |

---

*v2.1.0 | Dec 5, 2025*
