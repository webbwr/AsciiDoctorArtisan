...
ERROR_MESSAGES = {
    ...
    "languagetool_init_failed": (
        "Failed to initialize LanguageTool server. " "Ensure Java 8+ is installed."
    ),
    "ollama_not_installed": (
        "Ollama library not found. " "Install with: pip install ollama"
    ),
    "ollama_service_down": ("Ollama service not running. " "Start with: ollama serve"),
    "ollama_model_not_found": (
        "Ollama model not found. " "Pull with: ollama pull {model}"
    ),
    "document_too_large": (
    ...
    "check_timeout": (
        "Grammar check timed out after {timeout}ms. " "Try checking a smaller section."
    ),
}
