...
**Target Users**: Technical writers, documentation engineers, content creators, software developers who need professional AsciiDoc authoring tools.
...
**Independent Test**: Can be fully tested by launching the application, typing AsciiDoc content in the editor pane, and verifying that the preview pane updates within 350ms showing properly rendered HTML. Delivers immediate value as a standalone AsciiDoc editor with preview.

**Acceptance Scenarios**:

1. **Given** the application is running with an empty document, **When** the user types AsciiDoc content including headings, lists, and formatting, **Then** the preview pane displays the correctly rendered HTML within 350ms
...
4. **Given** the user creates a new document, **When** they save for the first time, **Then** a save dialog appears allowing them to choose location and filename with .adoc extension default
...
A software developer maintains documentation alongside code in a Git repository. They edit documentation in AsciiDoc Artisan, and when ready to commit changes, they select Git→Commit, enter a commit message, and the application stages and commits the current file. They can also pull latest changes and push to the remote repository without leaving the editor.

**Why this priority**: Version control integration eliminates context switching to terminal or separate Git GUI tools. For developers, this is a high-value workflow enhancement but not essential for basic editing.
...
A user closes AsciiDoc Artisan after working on documentation. The next time they launch the application, it automatically reopens the last file they were editing, restores the window size and position, and applies their preferred theme (dark/light). Their font size zoom level and splitter position between editor and preview are preserved.

**Why this priority**: Session persistence significantly improves user experience and workflow continuity but is not essential for basic functionality. Users can manually reopen files and reconfigure layout if needed.

**Independent Test**: Can be tested by configuring window layout, zoom level, and theme, closing the application, and verifying all settings restore correctly on next launch. Delivers comfort and efficiency improvements.
...
2. **Given** the user has set a custom window size and position, **When** they relaunch, **Then** the window restores to the same dimensions and screen location
...
An experienced user relies on keyboard shortcuts for efficient workflow. They use Ctrl+N for new document, Ctrl+O to open, Ctrl+S to save, Ctrl+F to find text, Ctrl+G to go to a specific line, Ctrl+D to toggle dark mode, and Ctrl+/- to adjust font zoom. All shortcuts follow platform conventions (Cmd on macOS, Ctrl on Windows/Linux).
...
- **What happens when the user opens a corrupt or invalid AsciiDoc file?**
  The application displays the raw content in the editor and shows an error message in the preview pane indicating parsing failure with details about the issue location.

- **How does the system handle opening extremely large files (>100MB)?**
  The application displays a warning about potential performance impact and offers to open in read-only mode with preview disabled, or to proceed with standard editing (may be slow).

- **What happens when Pandoc is not installed but the user tries to open a DOCX file?**
  The application detects the missing dependency on startup and when attempting conversion, displays a clear error message with installation instructions and a link to Pandoc download page.
...
- **FR-002**: System MUST display a live HTML preview of the AsciiDoc content updated within 350ms of user input
- **FR-003**: System MUST support word wrap and configurable tab stops (4 spaces default)
- **FR-004**: System MUST debounce preview updates to prevent UI blocking during rapid typing
- **FR-005**: System MUST maintain scroll synchronization between editor and preview panes
- **FR-006**: System MUST provide find and replace functionality with Ctrl+F keyboard shortcut
- **FR-007**: System MUST provide "go to line" functionality with Ctrl+G keyboard shortcut

#### File Operations

- **FR-008**: System MUST support opening .adoc and .asciidoc files natively
- **FR-009**: System MUST perform atomic writes to prevent file corruption during save operations
- **FR-010**: System MUST prompt users to save unsaved changes before closing documents or application
- **FR-011**: System MUST track file modification state and display indicator in window title
- **FR-012**: System MUST handle platform-specific path separators and line endings transparently
- **FR-013**: System MUST validate file paths to prevent directory traversal attacks
- **FR-014**: System MUST provide "Save As" functionality to save documents to new locations

#### Format Conversion

- **FR-015**: System MUST integrate with Pandoc to support document conversion
- **FR-016**: System MUST support importing DOCX, Markdown, HTML, LaTeX, reStructuredText, Org, and Textile formats
- **FR-017**: System MUST automatically detect file format and convert to AsciiDoc on open
- **FR-018**: System MUST support exporting to HTML, DOCX, and PDF formats via Pandoc
- **FR-019**: System MUST handle Pandoc errors gracefully with informative user messages
- **FR-020**: System MUST check for Pandoc availability on startup and display warnings if missing
- **FR-021**: System MUST preserve semantic structure (headings, lists, tables) during format conversion with graceful degradation for unsupported elements

#### Git Integration

- **FR-022**: System MUST detect if the current file is in a Git repository
- **FR-023**: System MUST provide Git commit functionality accessible via menu (Git→Commit)
- **FR-024**: System MUST stage and commit the current file with user-provided commit message
- **FR-025**: System MUST support Git pull operations to fetch remote changes
- **FR-026**: System MUST support Git push operations to publish local commits
- **FR-027**: System MUST never execute destructive Git operations (force push, hard reset) without explicit user confirmation
- **FR-028**: System MUST disable Git menu options when file is not in a repository

#### User Interface & Experience

- **FR-029**: System MUST provide both light and dark theme options
- **FR-030**: System MUST support theme toggle via keyboard shortcut (F5 or Ctrl+D)
- **FR-031**: System MUST meet WCAG AA contrast ratio requirements for both themes
- **FR-032**: System MUST provide resizable splitter between editor and preview panes
- **FR-033**: System MUST support font zoom via Ctrl+Plus, Ctrl+Minus, Ctrl+0 keyboard shortcuts
- **FR-034**: System MUST provide keyboard shortcuts following platform conventions (Ctrl on Windows/Linux, Cmd on macOS)
- **FR-035**: System MUST render correctly on high-DPI displays with proper scaling
- **FR-036**: System MUST display status bar with current file information

#### Session & Configuration

- **FR-037**: System MUST persist user preferences using platform-appropriate configuration directories (XDG on Linux, AppData on Windows, Application Support on macOS)
- **FR-038**: System MUST remember last opened file path and reopen on application launch
- **FR-039**: System MUST save and restore window geometry (size, position, maximized state)
- **FR-040**: System MUST persist user theme preference (light/dark)
- **FR-041**: System MUST save and restore editor font zoom level
- **FR-042**: System MUST persist splitter position between editor and preview panes
- **FR-043**: Configuration file MUST use JSON format for human readability and editability

#### Performance & Reliability

- **FR-044**: System MUST remain responsive when editing documents up to 10,000 lines
- **FR-045**: System MUST start within 3 seconds on standard hardware
- **FR-046**: System MUST use worker threads for CPU-intensive operations (Git, Pandoc, AsciiDoc rendering)
- **FR-047**: System MUST handle memory efficiently to prevent leaks during extended sessions
- **FR-048**: System MUST provide comprehensive error logging for debugging without exposing sensitive data

#### Cross-Platform Compatibility

- **FR-049**: System MUST function identically on Linux, macOS, and Windows platforms
- **FR-050**: System MUST support WSL (Windows Subsystem for Linux) with WSLg for GUI display
- **FR-051**: System MUST handle platform-specific file path conventions transparently
- **FR-052**: System MUST adapt keyboard shortcuts to platform conventions (Ctrl vs Cmd)
...
- **PreviewState**: Manages rendered HTML content, scroll position, and synchronization status with editor
- **Configuration**: Stores user preferences including theme, last opened file, window geometry, and persistent settings
- **GitRepository**: Represents detected Git repository with path, remote configuration, and current status
...
- **SC-001**: Users can edit documents with live preview updates occurring within 350ms of input
...
- **Advanced Git features**: Branching, merging, rebasing handled outside the application
...
**Next Steps**: Refer to constitution for development standards; Use `/speckit.plan` to generate implementation plan
