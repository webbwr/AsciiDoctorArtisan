...
def get_actual_metrics():
    ...
    # Get test count
    try:
        result = subprocess.run(["pytest", "--collect-only", "-q"], capture_output=True, text=True, timeout=10)
...
def compare_metrics(actual: dict, documented: dict) -> dict:
    ...
    """
    results = {"mismatches": [], "missing": [], "version_mismatch": False}
    ...
    if "test_count" in actual and "test_count" in documented:
        ...
        if abs(diff) > 100:  # Significant difference
            results["mismatches"].append(
                {"metric": "Test Count", "documented": doc_tests, "actual": act_tests, "difference": f"{diff:+d} tests"}
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Analyze architecture documentation gaps")
    parser.add_argument("--file", default="docs/developer/architecture.md", help="Path to architecture.md")
