...
def analyze_results(results):
    ...
    if mb_size > 0:
        ...
        if write_per_mb > 100:
            print(f"  ⚠️  Write may exceed 100ms target for 1MB files")
            print(f"     Recommendation: Use async I/O, implement write buffering")
        else:
            print(f"  ✅ Write performance meets 100ms target")

        if read_per_mb > 100:
            print(f"  ⚠️  Read may exceed 100ms target for 1MB files")
            print(f"     Recommendation: Use async I/O, implement read caching")
        else:
            print(f"  ✅ Read performance meets 100ms target")
