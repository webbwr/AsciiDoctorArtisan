...
└──────────────┬──────────────────────────────────┘
               ...
               ▼
┌─────────────────────────────────────────────────┐
│       AutoCompleteManager (Controller)          │
│  - Manages QCompleter lifecycle                 │
│  - Coordinates completion sources               │
│  - Filters and ranks suggestions                │
│  - Handles selection and insertion              │
└──────────────┬──────────────────────────────────┘
               │ query_completions(context)
               ▼
┌─────────────────────────────────────────────────┐
│      CompletionEngine (Business Logic)          │
│  - Context analysis (syntax, position)          │
...
└──────────────┬──────────────────────────────────┘
      ...
      ▼                 ▼             ▼          ▼
┌──────────────┐ ┌──────────────┐ ┌──────┐ ┌────────┐
│SyntaxProvider│ │AttrProvider  │ │Xref  │ │Include │
│- Headings    │ │- Doc attrs   │ │- IDs │ │- Files │
│- Lists       │ │- Built-in    │ │      │ │        │
│- Blocks      │ │- Custom      │ │      │ │        │
└──────────────┘ └──────────────┘ └──────┘ └────────┘
```
...
│   │   ├── completion_context.py        # Context analysis
│   │   ├── completion_providers.py      # Provider implementations
│   │   ├── fuzzy_matcher.py             # Fuzzy matching algorithm
│   │   └── snippet_manager.py           # Snippet expansion logic
│   └── models.py                         # Add CompletionResult model
├── ui/
│   └── autocomplete_manager.py          # Qt integration layer
└── tests/unit/core/completion/
    ├── test_completion_engine.py        # Engine tests
    ├── test_completion_context.py       # Context analysis tests
    ├── test_completion_providers.py     # Provider tests
    ├── test_fuzzy_matcher.py            # Fuzzy matching tests
    └── test_snippet_manager.py          # Snippet tests
```
...
- `noteblock` → NOTE admonition
- `tipblock` → TIP admonition
- `warningblock` → WARNING admonition
- `cautionblock` → CAUTION admonition
- `importantblock` → IMPORTANT admonition
- `exampleblock` → Example block
- `sidebarblock` → Sidebar block
- `quoteblock` → Quote block with attribution
...
- Full workflow: trigger → query → display → select → insert
...
```
┌────────────────────────────────────┐
│ ● heading1       Heading Level 1   │ ← Selected
│ ● heading2       Heading Level 2   │
│ ● heading3       Heading Level 3   │
│ ◆ :author:       Document Attribute│
│ ◆ :revnumber:    Version Number    │
│ ▲ <<intro>>      Cross-Reference   │
...
- Test coverage: 100% ✅
- Memory usage: <5MB ✅
- Startup time: No regression (maintain <1.1s) ✅
