...
- **Unit tests:** 5,254 (100% pass rate)
- **E2E tests:** 3 (100% pass rate)
- **Overall:** 5,257 tests (100% pass rate)
- **Coverage:** 96%
...
**Goal:** Verify user-facing workflows work correctly.

**Current status:** 3 E2E tests, 100% pass rate
...
Tests:
- `test_create_edit_save_export_pdf` - Document creation workflow
- `test_open_find_replace_commit` - Find/replace with Git
- `test_template_customize_save_export` - Template workflow

**Note:** E2E tests use `@pytest.mark.forked` to run in separate processes, avoiding Qt threading issues.
...
@pytest.mark.forked
class TestWorkflow:
    def test_create_document(self, app_window, qtbot, tmp_path):
        """Test document creation workflow."""
        app_window.editor.setPlainText("= Test Document")
        qtbot.wait(100)  # Give Qt event loop time
        assert "Test Document" in app_window.editor.toPlainText()
```
