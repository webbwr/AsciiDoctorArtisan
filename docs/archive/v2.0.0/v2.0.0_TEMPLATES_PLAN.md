...
|--------|-------------|---------|
| `{{var}}` | Simple variable | `{{title}}` → "My Article" |
| `{{var:default}}` | With default | `{{author:Anonymous}}` → "Anonymous" |
...
└── models.py               # Template, TemplateVariable dataclasses

ui/
├── template_browser.py     # Template selection dialog
├── template_variable_dialog.py  # Variable input form
└── template_manager_ui.py  # Template management (create/edit/delete)
...
```
User → File → New from Template
    ↓
TemplateBrowser.show_dialog()
    ↓
User selects template → TemplateVariableDialog.show_dialog()
    ↓
User fills variables → OK
    ↓
...
- End-to-end: select template → fill vars → instantiate (3 tests)
...
2. 📋 Implement Phase 1: Core Engine (6-8h)
3. 📋 Implement Phase 2: Manager (4-6h)
4. 📋 Implement Phase 3: Browser UI (4-6h)
5. 📋 Implement Phase 4: Built-in Templates (2-4h)
