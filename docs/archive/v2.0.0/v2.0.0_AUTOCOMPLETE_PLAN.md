...
1. **Syntax Completion**
   ...
   - Blocks: `[source` → `[source,python]`, `[example]` → full example block
   - Inline: `link:` → `link:URL[text]`, `image:` → `image::path[]`
...
3. **Cross-Reference Completion**
   ...
   - Suggest in `<<>>` syntax: `<<an` → `<<anchor-id>>`
   - Fuzzy matching on anchor text

4. **Include Path Completion**
   - File system search: `include::` → show `.adoc` files
...
5. **Snippet Expansion**
   ...
   - `code` → source block with language
   - `adm` → admonition (NOTE, TIP, WARNING, etc.)
...
```
core/
├── autocomplete_engine.py      # Core matching and ranking logic
├── autocomplete_providers.py   # Content providers (Syntax, Attr, XRef, Include, Snippet)
└── models.py                   # CompletionItem, CompletionKind, CompletionContext

ui/
├── autocomplete_widget.py      # Qt popup list widget
└── autocomplete_manager.py     # Coordinator between editor and engine
```

### Data Flow

```
User types → Editor TextChanged
    ↓
...
**In Preferences Dialog (Tools → Preferences):**
...
2. 📋 Set up project structure (`core/`, `ui/`, `tests/`)
3. 📋 Implement Phase 1: Core Engine (8-10h)
4. 📋 Implement Phase 2: Providers (6-8h)
5. 📋 Implement Phase 3: Qt Integration (6-8h)
6. 📋 Implement Phase 4: Snippets (4-6h)
7. 📋 Write documentation
8. 📋 Integration testing with main app
